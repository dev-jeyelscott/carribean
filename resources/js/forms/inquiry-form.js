    ...
    return {
        ...
        init() {
            this.fulfillmentType =
                this.$el.dataset.initialFulfillment ||
                this.$el.dataset.defaultFulfillment ||
                "pickup";
            ...
            this.$watch("fulfillmentType", (value) => {
                ...
                const deliveryAddress = this.$el.querySelector(
        ...
        errorInsertionTarget(field) {
            const matchingFields = this.$el.querySelectorAll(
        ...
        setFieldError(field, message) {
            ...
            let error = this.$el.querySelector(
                `[data-inquiry-error-for="${CSS.escape(key)}"]`,
            );
            ...
            this.$el
                .querySelectorAll(`[name="${CSS.escape(key)}"]`)
                .forEach((matchingField) => {
                    matchingField.setAttribute("aria-invalid", "true");
                    const describedBy = new Set(
                        (matchingField.getAttribute("aria-describedby") || "")
                            .split(/\s+/)
                            .filter(Boolean),
                    );
                    describedBy.add(errorId);
                    matchingField.setAttribute(
                        "aria-describedby",
                        [...describedBy].join(" "),
                    );
                });
            ...
            if (!error) {
                ...
                this.errorInsertionTarget(field).insertAdjacentElement(
                    "afterend",
                    error,
                );
            }
        ...
        clearFieldError(field) {
            ...
            const error = this.$el.querySelector(
                `[data-inquiry-error-for="${CSS.escape(key)}"]`,
            );
        ...
        successTitle() {
            ...
            const sourcePage = this.$el.querySelector(
                '[name="source_page"]',
            )?.value;
        ...
        resetAfterSuccess() {
            ...
            this.popup = null;
            this.fulfillmentType =
                this.$el.dataset.defaultFulfillment || "pickup";
            ...
            try {
                ...
                if (response.status === 419) {
                    this.showNotification(
                        "error",
                        "Session expired",
                        "Refresh the page and submit your inquiry again.",
                    );
                ...
                if (response.status === 429) {
                    this.showNotification(
                        ...
                        "Too many requests were received. Your inquiry has not been submitted.",
                    );
                ...
                if (
                    ...
                    this.showNotification(
                        ...
                        "The server could not process your inquiry. Your information has been preserved; please try again.",
                    );
